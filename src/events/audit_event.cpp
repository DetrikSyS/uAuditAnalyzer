...
Json::Value Audit_Event::getJSON()
{
    ...
    x["eventId"]["unixTime"] = (Json::Int64) unixTime;
    x["eventId"]["msecs"] = std::get<1>(eventId);
    x["eventId"]["id"] = (Json::Int64) std::get<2>(eventId);
    ...
    for (const string & className : classes)
    {
        if (isMultiContainer(className))
        {
            ...
            {
                x["classes"][className]["multi"] = true;
                x["classes"][className]["items"][i] = it->second->getJSON();
                i++;
